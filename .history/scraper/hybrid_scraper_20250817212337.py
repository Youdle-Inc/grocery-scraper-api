...
class TargetScraper(BaseStoreScraper):
    ...
    async def get_product_details(self, product_name: str, brand: str = None, store_name: str = None) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                'image_url': f"https://target.scene7.com/is/image/Target/GUEST_{hash(product_name) % 10000000000000000000}?wid=1200&hei=1200&qlt=80&fmt=webp",
                'product_url': f"https://www.target.com/p/{product_name.lower().replace(' ', '-').replace('/', '-')}/-/A-{hash(product_name) % 100000000}",
