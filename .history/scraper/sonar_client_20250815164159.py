...
Please provide a comprehensive list including:
1. Store name (e.g., Giant Eagle, Wegmans, ALDI, Walmart, Target, Kroger, Safeway, etc.)
2. Store address
3. Store website or online ordering URL
4. Available services (delivery, pickup, curbside, in-store shopping)
5. Store hours (if available)
6. Store status (open, temporarily closed, etc.)

Focus on major grocery chains and supermarkets. Return the information in a structured, easy-to-parse format.
Include both national chains and local grocery stores that serve this area."""
    ...
    def _extract_structured_stores(self, response: str, zipcode: str) -> List[StoreLocation]:
        ...
        # Look for numbered lists or structured formats
        ...
        for line in lines:
            ...
            # Look for store names in various formats
            store_match = self._extract_store_name(line)
            if store_match:
                if current_store:
                    stores.append(current_store)
                
                store_id = store_match.lower().replace(' ', '').replace('\'', '').replace('-', '')
            ...
            # Extract address
            if current_store and self._is_address_line(line):
            ...
            if current_store:
                ...
                if services:
                    ...
                    current_store.services.extend(services)
