...
Focus on current availability, accurate pricing, and finding actual product images from the store's website."""
            ...
            products = self._parse_product_response(response)
            
            # Enhance products with hybrid scraping (images + additional data) - OPTIONAL
            # Only enable if explicitly requested or for specific use cases
            enable_hybrid = os.environ.get("ENABLE_HYBRID_SCRAPING", "false").lower() == "true"
            ...
            if enable_hybrid:
                ...
                logger.info("⚡ Hybrid scraping disabled for faster response")
