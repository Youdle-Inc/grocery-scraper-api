...
class SonarClient:
    ...
    def __init__(self, api_key: Optional[str] = None):
        ...
        if Perplexity is None:
            logger.warning("⚠️ Perplexity SDK not installed. Sonar features will be disabled.")
            return
    ...
    async def _make_sonar_request(self, query: str) -> str:
        """Make a request to Perplexity Sonar"""
        try:
            # Use the Perplexity client to make the request
            response = await asyncio.get_event_loop().run_in_executor(
                None,
                lambda: self.client.chat(query)
            )
