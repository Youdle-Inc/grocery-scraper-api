...
### Product Search
```http
GET /products/search?query={product}&zipcode={zipcode}
```
...
1. **User Request**: Search for "oat milk" in ZIP code 10001
2. **Exa API**: AI finds products with names, prices, descriptions
3. **Structured Data**: Gets real URLs and images from web search
4. **Smart Matching**: Matches products by name similarity
5. **Response**: Returns structured data with real URLs and images

### 2. Store Discovery Flow
1. **User Request**: Find stores in ZIP code "60601"
2. **Exa API**: AI discovers grocery stores in the area
