...
from scraper.exa_py_integration import ExaPyClient
...
async def root():
    ...
    return {
        ...
        "endpoints": {
            ...
            "exa_products": "/exa/products/search"
        },
        "features": [
...
async def health_check():
    ...
    return {
        ...
        "services": {
            ...
            "exa_py": "available" if exa_py_client.is_available() else "unavailable"
        }
    }
...
@app.get("/exa/products/search", tags=["exa"], response_model=Dict[str, Any], response_model_exclude_none=True)
async def search_exa_products(query: str, organic: Optional[bool] = None, num_results: int = 10):
    """Exa-only product search using exa_py SDK, no Sonar involved."""
    try:
        if not exa_py_client.is_available():
            raise HTTPException(status_code=503, detail="Exa API not available - set EXA_API_KEY")

        filters = {"organic": bool(organic)} if organic is not None else None
        results = exa_py_client.search_grocery_products(query, filters=filters, num_results=num_results)
        products = ExaPyClient.extract_product_images(results)
        ...
        logger.error(f"❌ Exa-only search failed: {e}")
